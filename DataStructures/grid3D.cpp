    ...
    for(int i = 0; i< cells.size(); i++)
    {
        for(int j = 0; j< cells[i].size(); j++)
        {
            for(int k = 0; k< cells[i][j].size(); k++)
            {
				...
				cells[i][j][k]->setType(UNTYPED);

				if(cells[i][j][k]->data)
				{
					cells[i][j][k]->data->clearAll();
					delete cells[i][j][k]->data;
					cells[i][j][k]->data = NULL;
				}
            }
        }
    }
...
int grid3d::typeCells(SurfaceGraph* mesh)
{
    int TypedCells = 0;
...
int grid3d::typeCells(SurfaceGraph* mesh, int triIdx)
{
    ...
    for(int i = 1; i< div1 ; i++) // Saltamos el 0 porque es el mismo vertice.
    {
        ...
        Vector3d maxPt = v2Dir*((float)i/(float)div1)*v2Norm + points[largeIdx]; // Suponemos que al ser triangulo rectangulo mantiene proporciones.
    ...
    for(int i = 1; i< div2 ; i++) // Saltamos el 0 porque es el mismo vertice.
    {
        ...
        Vector3d maxPt = v3Dir*((float)i/(float)div2)*v3Norm + points[(largeIdx+1)%3]; // Suponemos que al ser triángulo rectángulo mantiene proporciones.
